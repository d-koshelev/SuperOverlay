...
public sealed class LayoutMutationCore
{    private readonly LayoutDocumentEditor _editor = new();
    ...
    public bool SetZIndex(ref LayoutDocument document, IReadOnlyDictionary<Guid, int> zMap)
    {
        ArgumentNullException.ThrowIfNull(document);
        ArgumentNullException.ThrowIfNull(zMap);
        ...
        var changed = false;
        foreach (var placement in document.Placements.ToList())
        {
            if (!zMap.TryGetValue(placement.ItemId, out var z) || placement.ZIndex == z)
            {
                continue;
            }
